...
def moveToPosition(curPos,destPos, state):
    temp = tuple(map(sub, curPos, destPos))
    print(temp[0])
    if temp[0] > 0:
        legal = api.legalActions(state) 
        return api.makeMove("West", legal)   
...
# HungryAgent
#
# Pacman eats the closest food
class HungryAgent(Agent):
    def getAction(self, state):
        foodPosList = api.food(state)
        curPos = api.whereAmI(state)               
        

        #find closest food position
        closestFoodPos = getClosestFoodPosition(curPos,foodPosList)                   
        
        #move in direction of food               
        # moveToPosition(curPos, (5,1), state)
        temp = tuple(map(sub, curPos, (5,1)))
